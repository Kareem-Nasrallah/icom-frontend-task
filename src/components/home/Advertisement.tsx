...
const Advertisement = ({ index }: { index: number }) => {
  const imgExt = index === 0 ? "png" : "jpg";
  return (
    <div
      className="relative rounded-[12px] w-[514px]! shrink-0 h-[599px] bg-cover bg-center bg-no-repeat"
      style={{
        backgroundImage: `url('/realState/r${index + 1}.${imgExt}')`,
      }}
    >

      {/* Top Icons: Call, WhatsApp, Favorite */}
      <div className="m-6 flex justify-between items-center">
        ...
        <Heart
          size={42}
          className="text-main bg-white dark:bg-slate-800 dark:hover:dark:bg-slate-700 real-estate-icon"
        />
      </div>

      {/* Details Card */}
      <div className="p-6 w-[calc(100%-48px)] rounded-[12px] gap-6 flex flex-col justify-end drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] absolute bottom-0 m-6 bg-card hover:bg-blue-200 dark:hover:bg-blue-900 transition-all duration-200">
